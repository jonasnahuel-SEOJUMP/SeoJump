...
/**
 * Dual NextAuth config:
 * - handlers (login + /api/auth/session): NO expone accessToken al navegador
 * - auth() server-side: sí incluye accessToken para llamar a Search Console
 *
 * El token queda cifrado en la cookie JWT; el endpoint de sesión del cliente
 * solo ve hasGscAccess / error.
 */
function buildAuthConfig(exposeAccessToken: boolean): NextAuthConfig {
  return {
    providers: [
      Google({
    ...
    callbacks: {
      ...
      async session({ session, token }) {
        const hasToken = Boolean(token.accessToken) && token.error !== 'RefreshAccessTokenError'
        session.hasGscAccess = hasToken
        session.error = token.error as string | undefined
        // Nunca mandar el bearer de Google al cliente vía /api/auth/session
...
// Cliente + rutas Auth: sesión sin accessToken
export const { handlers, signIn, signOut } = NextAuth(buildAuthConfig(false))

// Server actions / Route Handlers: auth() con accessToken para GSC
export const { auth } = NextAuth(buildAuthConfig(true))
